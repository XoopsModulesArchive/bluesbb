...
if (!class_exists('XoopsBluesTicket')) {
    class XoopsBluesTicket
    {
        ...
        {
            return ['XOOPS_BLUES_TICKET' => $this->issue($salt, $timeout)];
        }
        ...
        public function issue($salt = '', $timeout = 1800)
        {
            ...
            if (empty($_SESSION['XOOPS_BLUES_STUBS'])) {
                $_SESSION['XOOPS_BLUES_STUBS'] = [];
            }

            // limit max stubs 10

            if (count($_SESSION['XOOPS_BLUES_STUBS']) > 10) {
...
'token' => $token,
        ...
        public function i_check($post = true)
        {
            $this->_errors = [];
    ...
    $GLOBALS['xoopsBluesTicket'] = new XoopsBluesTicket();
}
